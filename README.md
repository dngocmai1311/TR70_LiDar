...
### v3.2 (2026-09-15)
- Thêm global 2D Occupancy Grid Map 40 m × 40 m, độ phân giải 0.05 m/cell.
- Dùng world XYZ/origin từ pipeline Fusion, ray casting Bresenham và confidence decay.
- Nút **Grid Map** mở bản đồ; **Clear Map** reset riêng bản đồ, không ảnh hưởng Zone.
- Overlay vị trí/hướng LiDAR, ROI world và thống kê FREE/OCCUPIED/UNKNOWN.
- Grid Map hỗ trợ zoom tại con trỏ (0.5x–20x), kéo trái để pan, kéo phải để rotate, Reset View và Fit Data.
- Cluster mode tạo fused object riêng cho từng ROI Fusion bằng DBSCAN WORLD XY; object giữ world Z, local XY, timestamp và số point theo từng LiDAR.
- Chế độ **Slices** thực hiện ROI filter → DBSCAN local XY riêng từng LiDAR → SliceObject, gồm local/world center, bbox và convex hull để chuẩn bị association ở bước sau.
- Chế độ **Associate** ghép SliceObject khác sensor theo ROI/time/WORLD XY bằng score cấu hình được, giữ singleton và hiển thị MultiSliceObject trong 2D/3D Fusion.
- **3D Reconstruction** resample/alignment measured contours và dựng estimated surface bán trong suốt; Wireframe cho phép kiểm tra correspondence. Estimated volume được giữ `null` vì mesh chưa được kiểm chứng topology.
