...
export class Lidar3D {
  constructor(host, panel, palette) {
    ...
    // Measured SliceObjects va duong association debug; khong tao surface/solid.
    this.associationGroup = new THREE.Group();
    this.associationGroup.name = 'slice-associations';
    this.scene.add(this.associationGroup);
    this.reconstructionGroup = new THREE.Group();
    this.reconstructionGroup.name = 'estimated-reconstructions';
  ...
  clearMultiSliceObjects() {
    for (const child of [...this.associationGroup.children]) {
      this.associationGroup.remove(child);
      this.disposeObject(child);
    }
  }

  setMultiSliceObjects(objects = []) {
    this.clearMultiSliceObjects();
    objects.forEach((object, objectIndex) => {
      const color = new THREE.Color().setHSL((objectIndex * 0.23 + 0.48) % 1, 0.85, 0.42);
      const centers = [];
      for (const slice of object.slices || []) {
        const contour = (slice.world?.contour || []).map(p => new THREE.Vector3(p.x, p.y, p.z));
        if (contour.length >= 2) {
          const geometry = new THREE.BufferGeometry().setFromPoints(contour);
          this.associationGroup.add(new THREE.LineLoop(geometry,
            new THREE.LineBasicMaterial({ color, transparent: true, opacity: 0.95 })));
        }
        const c = slice.world?.center || slice.world?.centroid;
        if (c) centers.push(new THREE.Vector3(c.x, c.y, c.z));
      }
      if (centers.length >= 2) {
        const hub = new THREE.Vector3(object.world.centroidXY.x, object.world.centroidXY.y,
          centers.reduce((sum, p) => sum + p.z, 0) / centers.length);
        const segments = [];
        centers.forEach(p => segments.push(p, hub));
        this.associationGroup.add(new THREE.LineSegments(new THREE.BufferGeometry().setFromPoints(segments),
          new THREE.LineDashedMaterial({ color, dashSize: 0.12, gapSize: 0.08, transparent: true, opacity: 0.7 })));
        this.associationGroup.children.at(-1).computeLineDistances();
      }
      if (centers.length) {
        const topZ = Math.max(...centers.map(p => p.z));
        const confidence = object.association.confidence == null ? 'single' : Math.round(object.association.confidence * 100) + '%';
        const label = this.label(object.multiObjectId + ' ' + object.sensorIds.map(id => 'L' + (id + 1)).join('+') + ' ' + confidence, '#007f7f');
        label.position.set(object.world.centroidXY.x, object.world.centroidXY.y, topZ + 0.35);
        this.associationGroup.add(label);
      }
    });
  }

  clearReconstructedObjects() {
    for (const child of [...this.reconstructionGroup.children]) {
      this.reconstructionGroup.remove(child);
      this.disposeObject(child);
    }
  }

  setReconstructedObjects(objects = [], options = {}) {
    this.clearReconstructedObjects();
    if (!options.showEstimatedSurface) return;
    objects.forEach((object, objectIndex) => {
      const reconstruction = object.reconstruction;
      if (!reconstruction || !reconstruction.status.startsWith('reconstructed-')) return;
      const mesh = reconstruction.mesh;
      if (!mesh.vertices.length || !mesh.indices.length) return;
      const positions = new Float32Array(mesh.vertices.length * 3);
      mesh.vertices.forEach((p, i) => { positions[i * 3] = p.x; positions[i * 3 + 1] = p.y; positions[i * 3 + 2] = p.z; });
      const geometry = new THREE.BufferGeometry();
      geometry.setAttribute('position', new THREE.BufferAttribute(positions, 3));
      geometry.setIndex(mesh.indices);geometry.computeVertexNormals();
      const color = new THREE.Color().setHSL((objectIndex * 0.23 + 0.56) % 1, 0.75, 0.48);
      const surface = new THREE.Mesh(geometry, new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.24,
        side: THREE.DoubleSide, depthWrite: false }));
      surface.renderOrder = 1;this.reconstructionGroup.add(surface);
      if (options.showWireframe) {
        const wire = new THREE.LineSegments(new THREE.WireframeGeometry(geometry),
          new THREE.LineBasicMaterial({ color, transparent: true, opacity: 0.75 }));
        wire.renderOrder = 2;this.reconstructionGroup.add(wire);
      }
      const box = reconstruction.estimated.bbox;
      const expectedSensors = window.lidarCount || object.sensorCount || reconstruction.usableSliceCount;
      const label = this.label(object.multiObjectId + ' Slices:' + reconstruction.usableSliceCount + '/' + expectedSensors + ' estimated', '#2855c5');
      label.position.set(reconstruction.estimated.centroid.x, reconstruction.estimated.centroid.y, box.maxZ + 0.55);
      this.reconstructionGroup.add(label);
    });
  }
